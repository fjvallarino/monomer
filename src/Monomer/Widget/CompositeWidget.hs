...
data CompositeTask e = CompositeTask e deriving Typeable

composite :: (Monad m, Eq s, Typeable s, Typeable e, Typeable m) => WidgetType -> s -> EventHandlerC s e ep -> UIBuilderC s e m -> WidgetInstance sp ep m
...
createComposite :: (Monad m, Eq s, Typeable s, Typeable e, Typeable m) => Composite s e ep m -> CompositeState s e m -> Widget sp ep m
...
compositeMerge :: (Monad m, Eq s, Typeable s, Typeable e, Typeable m) => Composite s e ep m -> CompositeState s e m -> sp -> WidgetInstance sp ep m -> WidgetInstance sp ep m -> WidgetInstance sp ep m
...
compositeHandleEvent :: (Monad m, Eq s, Typeable s, Typeable e, Typeable m) => Composite s e ep m -> CompositeState s e m -> PathContext -> SystemEvent -> sp -> WidgetInstance sp ep m -> Maybe (EventResult sp ep m)
compositeHandleEvent comp state ctx evt pApp widgetComposite = fmap processEvent result where
  ...
  processEvent (EventResult reqs evts evtsRoot) = EventResult newReqs messages newInstance where
    evtStates = getUpdateUserStates reqs
    evtApp = compose evtStates app
    newReqs = convertRequests reqs <> convertTasksToRequests tasks
    ...
    newRoot = _widgetResize (_instanceWidget tempRoot) app viewport renderArea tempRoot (_sizeReqC comp)
...
reduceCompositeEvents appEventHandler app events = foldl' reducer (app, Seq.empty, Seq.empty) events where
  reducer (app, tasks, messages) event = case appEventHandler app event of
    ...
    TaskC newApp task -> (app, tasks |> task, messages)
    MessageC message -> (app, tasks, messages |> message)

convertTasksToRequests :: Typeable e => Seq (IO (Maybe e)) -> Seq (EventRequest sp)
convertTasksToRequests reqs = flip fmap reqs $ \req -> RunCustom rootPath (fmap CompositeTask req)
...
compositeResize :: (Monad m, Eq s, Typeable s, Typeable e, Typeable m) => Composite s e ep m -> CompositeState s e m -> sp -> Rect -> Rect -> WidgetInstance sp ep m -> Tree SizeReq -> WidgetInstance sp ep m
