...
createEventContext :: WidgetEnv s e -> Maybe Path -> Maybe Path -> Path -> SystemEvent -> WidgetInstance s e -> Maybe Path
createEventContext wenv latestPressed activeOverlay currentTarget systemEvent widgetRoot = case systemEvent of
    -- Keyboard
...
handleSystemEvent renderer wenv systemEvent currentTarget widgetRoot = do
  ...
  case createEventContext wenv latestPressed activeOverlay currentTarget systemEvent widgetRoot of
    ...
    Just target -> do
      ...
      let widgetResult = fromMaybe emptyResult $ _widgetHandleEvent widget wenv target systemEvent widgetRoot
      ...
      handleWidgetResult renderer wenv widgetResult
        >>= handleFocusChange renderer systemEvent stopProcessing
...
handleFocusChange :: (MonomerM s m) => Renderer m -> SystemEvent -> Bool -> HandlerStep s e -> m (HandlerStep s e)
...
handleClipboardGet :: (MonomerM s m) => Renderer m -> Seq (WidgetRequest s) -> HandlerStep s e -> m (HandlerStep s e)
...
handleSendMessages renderer reqs previousStep = foldM reducer previousStep reqs where
  reducer previousStep (SendMessage path message) = do
    ...
    let (wenv, events, widgetRoot) = previousStep
    let emptyResult = WidgetResult Seq.empty Seq.empty widgetRoot
    let widgetResult = fromMaybe emptyResult $ _widgetHandleMessage (_instanceWidget widgetRoot) wenv path message widgetRoot

    (newWenv, newEvents, newWidgetRoot) <- handleWidgetResult renderer wenv widgetResult
